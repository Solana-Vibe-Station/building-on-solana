...
import { BuySettings } from "@/utils/buy-settings";
...
export async function swapToken(token: string, buySettings: BuySettings): Promise<SwapInformation> {
  try {
    ...
    const swapResponse = await axios.post<any>(`https://lite-api.jup.ag/swap/v1//swap`, {
    ...
    return {
      success: false,
      message: `Swap error (${error.response?.status || "unknown"}): ${error.message}`,
      tx: null,
      token: "",
      amount: null,
      time: "0.00",
